...
// Tutorial Guiado Servicios — recorrido breve e ILUSTRATIVO de la
// administración de los servicios habilitados para el agendamiento: nombre,
// descripción, guardado/actualización y el listado de servicios disponibles.
// El usuario no completa campos ni guarda nada; al cerrar el recorrido no se
// creó ningún servicio.
//
...
import { TOUR_SERVICIOS_STEPS } from "@/lib/tourServiciosSteps";
...
const GRUPO_LABEL = "Servicios de agendamiento";
...
function buildMeta(indiceActivo) {
    const dots = TOUR_SERVICIOS_STEPS
        .map((_, index) => {
...
/**
 * Botón "Tutorial Guiado" de servicios. Recibe las mismas clases que los
 * botones que tenga al lado en el header (`className`, `claseIcono`,
 * `claseEtiqueta`) para quedar visualmente idéntico a ellos.
 */
export default function TutorialGuiadoServicios({
    ...
    const iniciarTour = useCallback(() => {
        ...
        const instancia = driver({
            ...
            // una acción real sobre los servicios).
            disableActiveInteraction: true,
            // Si un ancla no está en el DOM (los servicios todavía cargan), el
            // paso se salta solo en vez de dejar el tour congelado.
            ...
            onHighlightStarted: (elemento, paso) => {
                ...
                // con la altura del panel cerrado (mismo mecanismo que
                // TutorialGuiadoFichas).
                if (paso?.abrirDetalles && elemento?.tagName === "DETAILS" && !elemento.open) {
                    elemento.open = true;
            ...
            steps: TOUR_SERVICIOS_STEPS.map((step, index) => ({
                ...
                popover: {
                    ...
                    nextBtnText: index === TOUR_SERVICIOS_STEPS.length - 1 ? "Finalizar" : "Siguiente",
                    prevBtnText: "Atrás",
                },
    ...
    return (
        <button
            type="button"
            onClick={iniciarTour}
            aria-label={ariaLabel || "Iniciar el tutorial guiado de servicios de agendamiento"}
            className={className}
        >
