...
// Tutorial Guiado Productos — recorrido breve e ILUSTRATIVO de la gestión de
// prestaciones y servicios: formulario de registro, filtros de búsqueda y el
// listado general. El usuario no completa campos ni guarda nada; al cerrar el
// recorrido no se creó ningún registro.
//
...
import { TOUR_PRODUCTOS_STEPS } from "@/lib/tourProductosSteps";
...
const GRUPO_LABEL = "Prestaciones y servicios";
...
function buildMeta(indiceActivo) {
    const dots = TOUR_PRODUCTOS_STEPS
        .map((_, index) => {
...
/**
 * Botón "Tutorial Guiado" de prestaciones y servicios. Recibe las mismas
 * clases que los botones que tiene al lado en el header (`className`,
 * `claseIcono`, `claseEtiqueta`) para quedar visualmente idéntico a ellos.
 */
export default function TutorialGuiadoProductos({
    ...
    const iniciarTour = useCallback(() => {
        ...
        const instancia = driver({
            ...
            // del tour sería una acción real sobre el catálogo).
            ...
            steps: TOUR_PRODUCTOS_STEPS.map((step, index) => ({
                ...
                popover: {
                    ...
                    nextBtnText: index === TOUR_PRODUCTOS_STEPS.length - 1 ? "Finalizar" : "Siguiente",
                    prevBtnText: "Atrás",
                },
            })),
    ...
    return (
        <button
            type="button"
            onClick={iniciarTour}
            aria-label={ariaLabel || "Iniciar el tutorial guiado de prestaciones y servicios"}
            className={className}
        >
