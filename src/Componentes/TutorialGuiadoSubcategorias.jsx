...
// Tutorial Guiado Subcategorias — recorrido breve e ILUSTRATIVO de la gestión
// de subcategorías de una categoría: formulario de creación/edición, listado y
// el acceso a las sub-subcategorías. El usuario no escribe ni guarda nada; al
// cerrar el recorrido no se creó ninguna subcategoría.
//
...
import { TOUR_SUBCATEGORIAS_STEPS } from "@/lib/tourSubcategoriasSteps";
...
const GRUPO_LABEL = "Subcategorías";
...
function buildMeta(indiceActivo) {
    const dots = TOUR_SUBCATEGORIAS_STEPS
        .map((_, index) => {
...
/**
 * Botón "Tutorial Guiado" de subcategorías. Recibe las mismas clases que los
 * botones que tenga al lado en el header (`className`, `claseIcono`,
 * `claseEtiqueta`) para quedar visualmente idéntico a ellos.
 */
export default function TutorialGuiadoSubcategorias({
    ...
    const iniciarTour = useCallback(() => {
        ...
        const instancia = driver({
            ...
            // clic sobre el elemento (ingresar, actualizar o eliminar a mitad
            // del tour sería una acción real sobre las subcategorías).
            disableActiveInteraction: true,
            // Si un ancla no está en el DOM (los datos todavía cargan o no hay
            // subcategorías registradas), el paso se salta solo en vez de dejar
            // el tour congelado.
            ...
            steps: TOUR_SUBCATEGORIAS_STEPS.map((step, index) => ({
                ...
                popover: {
                    ...
                    nextBtnText: index === TOUR_SUBCATEGORIAS_STEPS.length - 1 ? "Finalizar" : "Siguiente",
                    prevBtnText: "Atrás",
                },
            })),
    ...
    return (
        <button
            type="button"
            onClick={iniciarTour}
            aria-label={ariaLabel || "Iniciar el tutorial guiado de subcategorías"}
            className={className}
        >
