...
// Tutorial Guiado Presupuesto — recorrido breve e ILUSTRATIVO del armado del
// presupuesto de tratamiento: datos del profesional y del paciente, catálogo
// de servicios, el presupuesto con sus observaciones y la descarga del PDF.
// El usuario no agrega servicios ni completa campos; al cerrar el recorrido
// el presupuesto sigue vacío.
//
...
import { TOUR_PRESUPUESTO_STEPS } from "@/lib/tourPresupuestoSteps";
...
const GRUPO_LABEL = "Presupuesto Rápido";
...
function buildMeta(indiceActivo) {
    const dots = TOUR_PRESUPUESTO_STEPS
        .map((_, index) => {
...
/**
 * Botón "Tutorial Guiado Presupuesto". Recibe las mismas clases que los
 * botones que tiene al lado en el header (`className`, `claseIcono`,
 * `claseEtiqueta`) para quedar visualmente idéntico a ellos.
 */
export default function TutorialGuiadoPresupuesto({
    ...
    const iniciarTour = useCallback(() => {
        ...
        const instancia = driver({
            ...
            // clic sobre el elemento (agregar servicios o descargar el PDF a
            // mitad del tour generaría un documento real).
            ...
            steps: TOUR_PRESUPUESTO_STEPS.map((step, index) => ({
                ...
                popover: {
                    ...
                    nextBtnText: index === TOUR_PRESUPUESTO_STEPS.length - 1 ? "Finalizar" : "Siguiente",
                    prevBtnText: "Atrás",
                },
            })),
    ...
    return (
        <button
            type="button"
            onClick={iniciarTour}
            aria-label={ariaLabel || "Iniciar el tutorial guiado del presupuesto de tratamiento"}
            className={className}
        >
