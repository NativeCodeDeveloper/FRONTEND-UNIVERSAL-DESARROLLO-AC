...
// Tutorial Guiado Cotizaciones — recorrido breve y de solo lectura por la
// página de Cotizaciones del paciente (crear cotizaciones, filtros del
// historial y las acciones de cada tarjeta).
//
// Es independiente del tour principal de TourContext: no navega a otras rutas
// ni exige condiciones de éxito (`esperar`), igual que TutorialGuiadoFichas.
// Comparte en cambio la misma librería (driver.js) y las mismas clases
// .ac-tour-* de globals.css, para que el popover se vea idéntico al del
// tutorial grande.

import { useCallback, useEffect, useRef } from "react";
import { driver } from "driver.js";
import "driver.js/dist/driver.css";
import { TOUR_COTIZACIONES_STEPS } from "@/lib/tourCotizacionesSteps";
...
const GRUPO_LABEL = "Cotizaciones del paciente";
...
function buildMeta(indiceActivo) {
    const dots = TOUR_COTIZACIONES_STEPS
        .map((_, index) => {
...
/**
 * Botón "Tutorial Guiado Cotizaciones". Recibe las mismas clases que
 * BotonVideoTutorial y TutorialGuiadoFichas (`className`, `claseIcono`,
 * `claseEtiqueta`) para quedar visualmente idéntico a los botones que tiene
 * al lado en el header de la página.
 */
export default function TutorialGuiadoCotizaciones({
    etiqueta = "Tutorial Guiado Cotizaciones",
    ariaLabel,
    className = "",
    ...
    const iniciarTour = useCallback(() => {
        ...
        const instancia = driver({
            ...
            // clic sobre el elemento (cambiar el estado o eliminar una
            // cotización a mitad del tour sería una acción real que el
            // usuario no está pidiendo hacer).
            disableActiveInteraction: true,
            // Si un ancla no está en el DOM (el paciente todavía carga, o no
            // tiene cotizaciones registradas y las tarjetas no se renderizan),
            // el paso se salta solo en vez de dejar el tour congelado.
            ...
            onHighlightStarted: (elemento) => {
                ...
                // popover a ambos lados del elemento. Corre antes de que driver
                // mida, igual que la apertura de <details> en
                // TutorialGuiadoFichas.
                elemento?.scrollIntoView({ block: "center", behavior: "instant" });
            },
            steps: TOUR_COTIZACIONES_STEPS.map((step, index) => ({
                ...
                popover: {
                    ...
                    nextBtnText: index === TOUR_COTIZACIONES_STEPS.length - 1 ? "Finalizar" : "Siguiente",
                    prevBtnText: "Atrás",
                },
            })),
    ...
    return (
        <button
            type="button"
            onClick={iniciarTour}
            aria-label={ariaLabel || "Iniciar el tutorial guiado de las cotizaciones del paciente"}
            className={className}
        >
