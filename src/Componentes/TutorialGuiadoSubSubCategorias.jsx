...
// Tutorial Guiado Sub-Subcategorias — recorrido breve e ILUSTRATIVO de la
// gestión de sub-subcategorías (último nivel del catálogo): formulario de
// creación/edición y listado. El usuario no escribe ni guarda nada; al cerrar
// el recorrido no se creó ninguna sub-subcategoría.
//
...
import { TOUR_SUBSUBCATEGORIAS_STEPS } from "@/lib/tourSubSubCategoriasSteps";
...
const GRUPO_LABEL = "Sub-subcategorías";
...
function buildMeta(indiceActivo) {
    const dots = TOUR_SUBSUBCATEGORIAS_STEPS
        .map((_, index) => {
...
/**
 * Botón "Tutorial Guiado" de sub-subcategorías. Recibe las mismas clases que
 * los botones que tenga al lado en el header (`className`, `claseIcono`,
 * `claseEtiqueta`) para quedar visualmente idéntico a ellos.
 */
export default function TutorialGuiadoSubSubCategorias({
    ...
    const iniciarTour = useCallback(() => {
        ...
        const instancia = driver({
            ...
            // del tour sería una acción real sobre las sub-subcategorías).
            ...
            steps: TOUR_SUBSUBCATEGORIAS_STEPS.map((step, index) => ({
                ...
                popover: {
                    ...
                    nextBtnText: index === TOUR_SUBSUBCATEGORIAS_STEPS.length - 1 ? "Finalizar" : "Siguiente",
                    prevBtnText: "Atrás",
                },
            })),
    ...
    return (
        <button
            type="button"
            onClick={iniciarTour}
            aria-label={ariaLabel || "Iniciar el tutorial guiado de sub-subcategorías"}
            className={className}
        >
