...
// Tutorial Guiado Categorias — recorrido breve e ILUSTRATIVO de la gestión de
// categorías: formulario de creación/edición y lista de categorías con sus
// acciones. El usuario no escribe ni guarda nada; al cerrar el recorrido no se
// creó ninguna categoría.
//
...
import { TOUR_CATEGORIAS_STEPS } from "@/lib/tourCategoriasSteps";
...
const GRUPO_LABEL = "Categorías del catálogo";
...
function buildMeta(indiceActivo) {
    const dots = TOUR_CATEGORIAS_STEPS
        .map((_, index) => {
...
/**
 * Botón "Tutorial Guiado" de categorías. Recibe las mismas clases que los
 * botones que tenga al lado en el header (`className`, `claseIcono`,
 * `claseEtiqueta`) para quedar visualmente idéntico a ellos.
 */
export default function TutorialGuiadoCategorias({
    ...
    const iniciarTour = useCallback(() => {
        ...
        const instancia = driver({
            ...
            // del tour sería una acción real sobre las categorías).
            ...
            steps: TOUR_CATEGORIAS_STEPS.map((step, index) => ({
                ...
                popover: {
                    ...
                    nextBtnText: index === TOUR_CATEGORIAS_STEPS.length - 1 ? "Finalizar" : "Siguiente",
                    prevBtnText: "Atrás",
                },
            })),
    ...
    return (
        <button
            type="button"
            onClick={iniciarTour}
            aria-label={ariaLabel || "Iniciar el tutorial guiado de categorías"}
            className={className}
        >
