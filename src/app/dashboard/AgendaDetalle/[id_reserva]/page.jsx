        ...
        <div className="min-h-screen bg-[#FAFAFB]">
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10">
                ...
                {dataReservaId.map(data => {
                    const token = getStateTokens(data.estadoReserva);
                    return (
                        <div key={data.id_reserva} className="mb-6">
                            <span className={"inline-flex items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-xs font-bold " + token.pill}>
                                <span className="h-1.5 w-1.5 rounded-full" style={{backgroundColor: token.dot}} />
                                {data.estadoReserva}
                            </span>
                        </div>
                    );
                        ...
                        <div className="p-5 md:p-6">
                            <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                                <Select value={estadoReserva} onValueChange={(value) => setEstadoReserva(value)}>
                                    <SelectTrigger className="w-full sm:w-[280px] bg-white border border-slate-200 text-slate-800 shadow-sm rounded-lg focus:ring-2 focus:ring-[#6E56CF]/20 focus:border-[#6E56CF]">
                                        <SelectValue placeholder="Selecciona un estado"/>
                                    </SelectTrigger>
                                    <SelectContent className="z-50 bg-white text-slate-800 border border-slate-200 shadow-xl rounded-xl">
                                        <SelectGroup>
                                            <SelectItem className="cursor-pointer rounded-lg focus:bg-violet-50" value="reservada">Reservada</SelectItem>
                                            <SelectItem className="cursor-pointer rounded-lg focus:bg-violet-50" value="confirmada">Confirmada</SelectItem>
                                            <SelectItem className="cursor-pointer rounded-lg focus:bg-violet-50" value="asiste">Asiste</SelectItem>
                                            <SelectItem className="cursor-pointer rounded-lg focus:bg-violet-50" value="no asiste">No Asiste</SelectItem>
                                            <SelectItem className="cursor-pointer rounded-lg focus:bg-violet-50" value="finalizado">Finalizado</SelectItem>
                                            <SelectItem className="cursor-pointer rounded-lg focus:bg-violet-50" value="anulada">Anulada</SelectItem>
                                            <SelectItem className="cursor-pointer rounded-lg focus:bg-violet-50" value="pendiente pago">Pendiente de Pago</SelectItem>
                                        </SelectGroup>
                                    </SelectContent>
                                </Select>
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white bg-black rounded-lg hover:bg-slate-800 transition-all duration-150 shadow-sm"
                                >
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center gap-1.5 px-4 py-2.5 text-sm font-semibold text-rose-600 bg-rose-50 border border-rose-200 rounded-lg hover:bg-rose-100 transition-all duration-150"
                                >
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            ...
                            {confirmarEliminacion && (
                                <div className="mt-4 rounded-lg border border-rose-200 bg-rose-50 p-4">
                                    <p className="text-sm font-semibold text-rose-800">Esta seguro de que desea eliminar esta cita?</p>
                                    <p className="mt-1 text-xs text-rose-600/80">Esta accion no se puede deshacer.</p>
                                    <div className="mt-3 flex gap-2">
                                        <button
                                            ...
                                            className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-rose-600 rounded-lg hover:bg-rose-700 transition-all shadow-sm"
                                        >
                                            Si, eliminar
                                        </button>
                                        <button
                            ...
                            {mensajeEliminacion && (
                                <div className="mt-4 rounded-lg border border-rose-200 bg-rose-50 px-4 py-3">
                                    <p className="text-sm font-semibold text-rose-700">{mensajeEliminacion}</p>
                                </div>
                            )}
                        </div>
                    </div>
